...
output_fpath = "test_toy.zarr"


def write_toy_adata() -> None:
    """create a small toy dataset to be used for debugging purposes"""
    if True:
    ...
    if not os.path.isdir(output_fpath):
        write_spatial_anndata(
            ...
            image_axes=["y", "x"],
            image=x,
            image_translation=np.array([translation_x, translation_y]).astype(float),
            image_scale_factors=np.array([scale_factor, scale_factor]),
            # # expression
            # tables_adata=a_cells,
            # tables_region="circles/circles_table",
            # circles
            circles_adata=a_circles,
            # # points
